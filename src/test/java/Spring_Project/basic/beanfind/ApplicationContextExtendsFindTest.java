...
public class ApplicationContextExtendsFindTest {
    ...
    @Test
    @DisplayName("부모 타입으로 조회시 자식 둘 이상이면 중복 오류가 발생")
...
//        DiscountPolicy bean = ac.getBean(DiscountPolicy.class);
    ...
    @Test
    @DisplayName("부모 타입으로 조회시 자식 둘 이상이면 빈 이름을 지정하면 됨")
    ...
    void findAllBeanByParentType() {
        ...
        for (String key : beansOfType.keySet()) {
            System.out.println("key = " + key + " value = " + beansOfType.get(key));    // 원래는 프린트를 하면 안됨
        }
    }
